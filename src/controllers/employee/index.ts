...
export const getAllEmployee = async (req, res) => {
  ...
  try {
    ...
    const countTotal = await countData(employeeModel, criteria);
    ...
    return res.status(HTTP_STATUS.OK).json(new apiResponse(HTTP_STATUS.OK, responseMessage?.getDataSuccess("Employee"), { employee_data: response, totalData: countTotal, state: stateObj }, {}));
  } catch (error) {
    console.error(error);
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(new apiResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, responseMessage?.internalServerError, {}, error));
  }
